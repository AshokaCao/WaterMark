...
- (void)setupUI
{
    ...
    minWidth = self.bounds.size.width * 1;
...
#warning clang - 用拖拽按钮控制父控件旋转
    ...
    backgroudImageView.image = [UIImage imageNamed:@"editor_back_group"];
...
//    pasterImageView.layer.borderColor = [UIColor whiteColor].CGColor;
//    pasterImageView.layer.borderWidth = 1.0;
    pasterImageView.image = [UIImage imageNamed:@"editor_back_group"];
    ...
    [delegateImageView setImage:[UIImage imageNamed:@"label_editor_delet"]];
    ...
    [scaleImageView setImage:[UIImage imageNamed:@"label_editor_size"]];
    ...
    self.pasterImageView.frame = CGRectMake(paster_insert_space, paster_insert_space, defaultImageViewW_H, 60);
    ...
    [editImageView setImage:[UIImage imageNamed:@"label_editor_size"]];
    UITapGestureRecognizer *editResizeGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(editClick:)] ;
    editImageView.userInteractionEnabled = YES;
    [editImageView addGestureRecognizer:editResizeGesture] ;
    [self addSubview:editImageView];
    self.editImageView = editImageView;
    self.editImageView.frame = CGRectMake(CGRectGetMaxX(self.pasterImageView.frame) - btnW_H/2, 0, btnW_H, btnW_H);
    
    
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.frame = self.pasterImageView.bounds;
    NSLog(@"pasterImage  -  %@",self.pasterImage);
//    [button setBackgroundImage:self.pasterImage forState:UIControlStateNormal];
    [button setImage:self.pasterImage forState:UIControlStateNormal];
    [button addTarget:self action:@selector(testClick:) forControlEvents:UIControlEventTouchUpInside];
    self.editBtn = button;
//    [self.pasterImageView addSubview:button];
...
- (void)editClick:(UITapGestureRecognizer *)edit
{
    [self.delegate editImageAction];
}
